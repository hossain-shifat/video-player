...
export default function Movies() {
    ...
    const filtered = useMemo(() => {
        let list = activeGenre === "All" ? movies : movies.filter((m) => m.metadata?.genres?.includes(activeGenre));
    ...
    return (
        <div className="space-y-6">
                        ...
                        {genres.map((g) => (
                            <button
                                ...
                                className={[
                                    ...
                                    activeGenre === g ? "bg-primary text-primary-content shadow-sm" : "text-base-content/80 hover:text-base-content hover:bg-base-300",
                                ].join(" ")}>
                                {g}
                            </button>
                        ))}
                        ...
                        {[
                            ...
                            <li key={opt.key}>
                                <button
                                    ...
                                    className={["text-xs rounded-md", sortBy === opt.key ? "text-primary font-semibold" : "text-base-content/90"].join(" ")}>
                                    {opt.label}
                                </button>
                            </li>
                        ))}
            ...
            {/* ── Grid ── */}
            {filtered.length === 0 ? (
                <div className="py-20 text-center text-base-content/60 text-sm">No movies in &ldquo;{activeGenre}&rdquo;</div>
            ) : (
                <>
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-7 gap-3">
                        {visible.map((item) => (
