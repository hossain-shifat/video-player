...
async function ingestAll(force = false) {
    if (_ingestRunning && !force) {
        console.log("[EPG] Ingest already running");
        return;
    }
    _ingestRunning = true;
    ...
    try {
        ...
        if (!sources.length) {
            console.log("[EPG] No enabled EPG sources");
            return;
        }
...
function refreshNowCache() {
    try {
        ...
        console.error("[EPG] refreshNowCache failed — keeping previous cache:", err.message);
    }
}

function getNowCache() {
    return _nowCache;
}
...
function refreshEventCache() {
    try {
        const liveRows = getLiveRows();
        const workingIds = new Set(liveRows.filter((r) => r.streamStatus === "working").map((r) => r.id));
        ...
        console.error("[EPG] refreshEventCache failed — keeping previous cache:", err.message);
        // Don't clear _eventsCache/_recoCache on failure — stale-but-valid data
        // is better than wiping the Live page's sports section to empty.
    }
}

function getEventsCache() {
    return _eventsCache || { featuredEvents: [], sports: [], news: [], entertainment: [], updatedAt: null };
}
function getRecoCache() {
    return _recoCache || { sports: [], news: [], kids: [], movies: [], featuredEvents: [], updatedAt: null };
}
    ...
    _timers.push(
        setInterval(() => {
            try {
                ...
                console.error("[EPG] now/event cache refresh failed:", err.message);
            }
        }, NOW_INTERVAL),
    );
